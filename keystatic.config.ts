  ...
  singletons: {
    ...
    services: singleton({
      ...
      schema: {
        ...
        pmSidebarTitle: fields.text({ label: 'Property Management — sidebar title', defaultValue: "Who it's for" }),
        pmSidebarItems: fields.array(fields.text({ label: 'Sidebar item' }), { label: 'Property Management — sidebar items', itemLabel: (p) => p.value || 'Item' }),
        ...
        acctSidebarTitle: fields.text({ label: 'Accounting — sidebar title', defaultValue: 'What you get' }),
        acctSidebarItems: fields.array(fields.text({ label: 'Sidebar item' }), { label: 'Accounting — sidebar items', itemLabel: (p) => p.value || 'Item' }),
        ...
        ownersRepSidebarTitle: fields.text({ label: "Owner's Rep — sidebar title", defaultValue: 'Common projects' }),
        ownersRepSidebarItems: fields.array(fields.text({ label: 'Sidebar item' }), { label: "Owner's Rep — sidebar items", itemLabel: (p) => p.value || 'Item' }),
        ...
        insuranceSidebarTitle: fields.text({ label: 'Insurance — sidebar title', defaultValue: 'Claim types we assist with' }),
        insuranceSidebarItems: fields.array(fields.text({ label: 'Sidebar item' }), { label: 'Insurance — sidebar items', itemLabel: (p) => p.value || 'Item' }),
        ...
        selfMgdHeadline: fields.text({ label: 'Self-Managed HOA Support — headline', defaultValue: 'Self-Managed HOA Support' }),
        selfMgdBody: fields.text({ label: 'Self-Managed HOA Support — body', multiline: true, defaultValue: "Some boards want to stay in the driver's seat — and should. We work behind the scenes so your volunteers can run the community without running the back office." }),
        selfMgdBullets: fields.array(fields.text({ label: 'Bullet' }), { label: 'Self-Managed HOA Support bullets', itemLabel: (p) => p.value || 'Bullet' }),
        selfMgdSidebarTitle: fields.text({ label: 'Self-Managed HOA Support — sidebar title', defaultValue: 'Add on when you need it' }),
        selfMgdSidebarItems: fields.array(fields.text({ label: 'Sidebar item' }), { label: 'Self-Managed HOA Support — sidebar items', itemLabel: (p) => p.value || 'Item' }),
      },
    }),
    ...
    site: singleton({
      ...
      schema: {
        ...
        contactPhoneCardTitle: fields.text({ label: 'Contact card — Phone title', defaultValue: 'Phone' }),
        contactPhoneNumber: fields.text({ label: 'Contact card — phone number', description: 'Shown on the card; the tap-to-call link is built from these digits.', defaultValue: '(435) 250-4150' }),
        contactHoursCardTitle: fields.text({ label: 'Contact card — Office Hours title', defaultValue: 'Office Hours' }),
        contactHoursBody: fields.text({ label: 'Contact card — office hours', multiline: true, description: 'One line per row.', defaultValue: 'Monday – Friday\n9:00 AM – 5:00 PM' }),
        contactPortalCardTitle: fields.text({ label: 'Contact card — Homeowner Portal title', defaultValue: 'Homeowner Portal' }),
        contactPortalBody: fields.text({ label: 'Contact card — Homeowner Portal body', multiline: true, defaultValue: 'For maintenance requests, documents, and payments.' }),
        contactPortalLinkLabel: fields.text({ label: 'Contact card — Homeowner Portal link text', defaultValue: 'Login to portal →' }),
      },
    }),
  },
});
